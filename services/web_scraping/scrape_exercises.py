...
class ScrapeExercises(BaseService):
    def __init__(self):
        super().__init__()
    ...
    def do(self):
        ...
        try:
            for exercise in exercises_list:
                ExerciseRepository().insert(name=exercise.get("name"), image_url=exercise.get("image_url"), instruction=exercise.get("instructions"),tags=exercise.get("tags"), created_by="scraper")
        except Exceptions as err:
            raise err

    @defer.inlineCallbacks
    def __crawl(self):
        yield self.runner.crawl(GetPostsURLSpider)
        yield self.runner.crawl(GetPostsDataSpider)
        reactor.stop()
